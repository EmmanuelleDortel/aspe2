...
#' df <- mef_creer_passerelle() %>%
#' mef_ajouter_ope_date_complete() %>%
#' filter(annee %in% 2010:2023, mois %in% 3:11) %>%
#' mef_filtrer_operation(var_id = pop_id,
#' var_tmp = annee,
#' var_date = mois) %>%
#' mef_ajouter_surf_calc() %>%
...
#' default = TRUE)
#' }
mef_filtrer_operation <- function(df,var_id,var_tmp,var_surf=NULL,var_pro=NULL,var_pds=NULL,var_mei=NULL,var_pas=NULL,var_date=NULL,default=FALSE) {
  #---------------------------------------------------------------------------
  var_id <- enquo(var_id)
  ...
  filter_function <- function(tb) {
    df_id <- df %>% right_join(select(tb, !!var_id)) %>% distinct()
    id <- df_id %>% right_join(tb) %>% pull(ope_id) %>% unique()
    #---------------------------------------------------------------------------
  ...
  ope_select <- unlist(lapply(1:nrow(ope_filter), function(i) filter_function(ope_filter[i,]))) %>%
    c(select(df, !!var_id, !!var_tmp, ope_id) %>% right_join(nb_ope[nb_ope$nb == 1,]) %>% pull(ope_id) %>% unique())
