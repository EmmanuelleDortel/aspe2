#' Reconstituer les effectifs et les poids par lots et taille
#'
#' @import magrittr
...
#'
#' @param df data frame (doit contenir les champs lop_id, tlo_id, tyl_id, lop_effectif et un champs identifiant les espèces)
...
#' }
mef_reconstituer_capture_par_taille <- function(df, var_taxon=esp_code_alternatif) {
  ...
  buid_size <- function(df_n,i) {
    ...
    d <- d %>% mutate(mei_taille = as.numeric(as.vector(d$mei_taille))) %>%
      ...
      mutate(lop_poids = unique(df_n$lop_poids[df_n$lop_id == i])) %>%
      mutate(lop_poids_estime = unique(df_n$lop_poids_estime[df_n$lop_id == i])) %>%
      select(lop_id, !!var_taxon, mei_taille, tlo_id, lop_effectif, lop_poids, lop_poids_estime, tai_effectif)
    return(d)
  }
  #-----------------------------------------------------------------------------
  # Add individual measurements and minimum and maximum lot sizes
  df_mei <- select(df, !!var_taxon, lop_id, tlo_id, tyl_id, lop_effectif, lop_poids, lop_poids_estime) %>%
    distinct() %>%
    ...
    mutate(mei_poids = replace(mei_poids, mei_poids == 0, NA),
           mei_poids_estime = replace(mei_poids_estime, mei_poids_estime == 0, NA))
  #-----------------------------------------------------------------------------
  ...
  df_tai <- df_mei %>%
    filter(tyl_id %in% 4) %>%
    mutate(mei_poids = ifelse(is.na(mei_poids), lop_poids, mei_poids),
           mei_poids_estime = ifelse(is.na(mei_poids_estime), lop_poids_estime, mei_poids_estime)) %>%
    mutate(across(c("lop_longueur_specimens_taille_mini","lop_longueur_specimens_taille_maxi"), ~mei_taille)) %>%
    mutate(tai_effectif = 1) %>%
    select(lop_id, !!var_taxon, mei_taille, tlo_id, lop_effectif, lop_poids, lop_poids_estime, tai_effectif, tai_poids = mei_poids, tai_poids_estime = mei_poids_estime, lop_longueur_specimens_taille_mini, lop_longueur_specimens_taille_maxi)
  #-----------------------------------------------------------------------------
  ...
  if (nrow(df_1) > 0) {
    df_tai <- df_tai %>%
      rbind(mutate(df_1, mei_effectif = 1) %>%
              summarise(tai_effectif = sum(mei_effectif),
                        tai_poids = sum(mei_poids),
                        tai_poids_estime = sum(mei_poids_estime),
                        .by = c(lop_id,!!var_taxon,mei_taille,tlo_id,lop_effectif,lop_poids,lop_poids_estime)) %>%
  ...
  if (nrow(df_2) > 0) {
    df_tai <- df_tai %>%
      rbind(do.call("rbind",lapply(unique(df_2$lop_id), function(i) buid_size(df_2,i))) %>%
              mutate(tai_poids = NA, tai_poids_estime = NA) %>%
  ...
  if (nrow(df_tai) > 0) {
    df_tai <- df_tai %>%
      ...
      mutate(tai_poids_taille = tai_effectif * (a * (mei_taille^b))) %>%
      select(-a, -b) %>%
      relocate(tai_poids_taille, .after = tai_poids_estime)
  }
  #-----------------------------------------------------------------------------
  ...
  if (nrow(df_3) > 0) {
    df_tai <- df_tai %>%
      rbind(mutate(df_3, tai_effectif = NA, mei_taille = NA, tai_poids = NA, tai_poids_estime = NA, tai_poids_taille = NA) %>%
              select(lop_id, !!var_taxon, mei_taille, tlo_id, lop_effectif, lop_poids, lop_poids_estime, tai_effectif, tai_poids, tai_poids_estime, tai_poids_taille, lop_longueur_specimens_taille_mini, lop_longueur_specimens_taille_maxi) %>%
              distinct())
  }
  #-----------------------------------------------------------------------------
