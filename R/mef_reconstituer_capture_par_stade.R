#' Reconstituer les effectifs et les poids par taxon et stade de vie
#'
#' @import magrittr
#' @rawNamespace import(dplyr, except = lag)
#' @importFrom rlang enquo
#'
#' @param df data frame (doit contenir les champs tlo_id, tai_effectif, tai_poids, tai_poids_estime, tai_poids_taille, lop_effectif, lop_poids, lop_poids_estime, mei_taille, lop_longueur_specimens_taille_mini, lop_longueur_specimens_taille_maxi)
...
#'  }
mef_reconstituer_capture_par_stade <- function(df, var_id=ope_id, var_taxon=esp_code_alternatif, var_tai=esp_taille_mat, var_tlo=tlo_id, tb=NULL) {
  var_id <- enquo(var_id)
  ...
  if (!"tai_poids_estime" %in% colnames(df)) { df$tai_poids_estime <- NA }
  ...
  if (nrow(df_1) > 0) {
    ...
    df_std <- df_std %>%
      rbind(filter(df_1, !is.na(df_1$stade)) %>% mutate(tai_effectif = lop_effectif, tai_poids = lop_poids, tai_poids_estime = lop_poids_estime) %>% distinct()) %>%
  ...
  df_std <- df_std %>%
    summarise(std_effectif = sum(tai_effectif),
              std_poids = sum(tai_poids),
              std_poids_estime = sum(tai_poids_estime),
              std_poids_taille = sum(tai_poids_taille),
              .by = c(!!var_id, !!var_taxon, stade))
  #-----------------------------------------------------------------------------
  # Add missing life-stage
  df_std <- df_std %>%
    rbind(select(df_std, !!var_id, !!var_taxon) %>% distinct() %>%
            ...
            mutate(std_effectif = 0, std_poids = 0, , std_poids_estime = 0, std_poids_taille = 0))
  #-----------------------------------------------------------------------------
  ...
  df_std <- df_std %>%
    rbind(filter(df, is.na(tai_stade)) %>%
            ...
            distinct() %>%
            mutate(stade = NA, std_effectif = NA, std_poids = NA, std_poids_estime = NA, std_poids_taille = NA))
  #-----------------------------------------------------------------------------
  return(df_std)
}
